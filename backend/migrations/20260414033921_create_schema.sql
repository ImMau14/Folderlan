...
CREATE TABLE IF NOT EXISTS Users (
    ...
    can_delete_own_files INTEGER NOT NULL DEFAULT 0,
    has_upload_limits INTEGER NOT NULL DEFAULT 0,
...
-- Table For Managing File Permissions
...
-- Indexes For Faster Permission Queries
...
-- Trigger To Ensure Only One Owner Exists
...
-- Trigger To Log Permission Grants
...
-- Trigger To Log Permission Updates
...
-- Trigger To Log Permission Revocations
...
-- Trigger To Log User Permission Updates
...
-- Trigger To Log User Soft Deletes
...
-- Trigger To Log File Soft Deletes
...
-- Trigger To Prevent Direct User Deletion
...
-- Trigger To Prevent Direct File Deletion
...
CREATE TRIGGER IF NOT EXISTS trg_audit_file_insert
...
BEGIN
    ...
    VALUES (
        NEW.uploaded_by,
        'FILE_UPLOAD',
        'File uploaded: ' || NEW.name || ' (' || NEW.size_bytes || ' bytes)',
        NEW.id
    );
END;
