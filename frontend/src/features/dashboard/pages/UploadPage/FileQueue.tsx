...
export const FileQueue: FC<FileQueueProps> = ({
  ...
  return (
    <FloatingContainer className="animate-fall-on-2 h-full overflow-hidden border border-ui-border brightness-[99%] filter lg:overflow-hidden">
      <div className="flex h-full w-full flex-col gap-4">
        {/* Cabecera */}
        <div className="flex shrink-0 items-start justify-between gap-4 sm:items-center">
        ...
        {hasFiles ? (
          <div className="stagger-group flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto pr-1 scrollbar scrollbar-rounded scrollbar-thin scrollbar-thumb-ui-text-muted">
            {files.map((file) => (
              <FileContainer
                key={file.key}
                file={file.file}
